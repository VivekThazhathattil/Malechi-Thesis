...
for row in weather_csv:
    if float(row[0]) >= 3 and float(row[0]) <= 22:
        rows.append(row)
...
plt.legend([l_min_temp, l_max_temp, l_num_rainy_days, l_rainfall,
    l_rh7, l_rh2], ["Min. temp", "Max. temp", "No. of rainy days",
        "rainfall (mm)", "RH I (%)", "RH II (%)"], loc='center left',
    bbox_to_anchor=(1.05, 0.5))
ax.set_xlabel("Standard Week", fontsize=12)
ax.set_title("Weather data during cropping period (January 2021 to June 2021)", fontsize=12)
plt.tight_layout()
plt.show()
